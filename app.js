...
function renderProducts(items) {
    if (!items || items.length === 0) {
        productGrid.innerHTML = '<p style="text-align:center; grid-column: 1/-1; padding: 5rem;">Configurando base de datos...</p>';
        return;
    }
...
function changeQuantity(pid, delta) {
    ...
    if (!item) return;
    item.quantity += delta;
    if (item.quantity <= 0) cart = cart.filter(c => String(c.id) !== String(pid));
    updateCartUI();
}
...
function toggleLoginModal(s) { loginModal.style.display = s ? 'block' : 'none'; modalOverlay.style.display = s ? 'block' : 'none'; if (s) adminPassInput.focus(); }
...
function handleHeaderScroll() {
    if (window.scrollY > 50) { header.style.padding = '0.5rem 0'; header.style.background = 'rgba(2, 6, 23, 0.95)'; }
    else { header.style.padding = '1rem 0'; header.style.background = 'rgba(30, 41, 59, 0.7)'; }
}
